...
/**
 * Model coverage — modules::documents
 *
 ...
 *
 *   MyDocument                   -> My Documents  [not in the registered model map]
 *   MyCompanyDocument            -> Company Documents  [not in the registered model map]
 *   MyPayslip                    -> Payslips  [not in the registered model map]
 *   CompanyDocument              -> reached through another tab's form
 *   EmployeeDocument             -> reached through another tab's form
 *   PayslipDocument              -> reached through another tab's form
 */

const MODULE = 'modules::documents';

const TABS = [
  { label: "My Documents", model: "MyDocument" },
  { label: "Company Documents", model: "MyCompanyDocument" },
  { label: "Payslips", model: "MyPayslip" },
];

test.describe('modules::documents — model lists', () => {
...
test.describe('modules::documents — create/delete round trip', () => {
  for (const { label, model } of TABS) {
