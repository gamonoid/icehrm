...
import DirectoryUserExtensionView from "./view";

class DirectoryUserModule extends ExtensionModuleBase {
  /**
  ...
  showExtensionView() {
    ...
    ReactDOM.render(<DirectoryUserExtensionView apiClient={window.modJs.apiClient} ice={window.modJs}/>, document.getElementById('content'));

    // Calling a method in the controller (just to show how it works)
    //window.directoryExtensionController.handleTestAction();

    // Sending some test api requests to endpoints defined in ApiController.php
    //window.directoryExtensionController.makeSomeTestApiRequests();
  }
}

export default DirectoryUserModule;
