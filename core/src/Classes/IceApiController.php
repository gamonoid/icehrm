...
class IceApiController extends RestEndPoint
{
	...
	{
		// Callers conventionally pass REST_API_PATH.'endpoint'. Strip it before
		// prefixing below: with REST_API_PATH '/' the doubling is collapsed by the
		// router, but on hosted it is '/<client>/' and the route would become
		// '/<client>/<client>/endpoint', which never matches.
		if (defined('REST_API_PATH') && REST_API_PATH !== '/' && strpos($path, REST_API_PATH) === 0) {
			$path = substr($path, strlen(REST_API_PATH));
		}
