...
$allowedExtensions = explode(',', "jpg,gif,png,jpeg");
...
$uploadFilesToS3Key = SettingsManager::getInstance()->getSetting("Files: Amazon S3 Key for Editor File Upload");
$uploadFilesToS3Secret = SettingsManager::getInstance()->getSetting(
	"Files: Amazon S3 Secret for Editor File Upload"
);
$s3Bucket = SettingsManager::getInstance()->getSetting("Files: S3 Bucket for Editor");
$s3WebUrl = SettingsManager::getInstance()->getSetting("Files: S3 Web Url for Editor");
...
if ($uploadFilesToS3.'' == '1' && !empty($uploadFilesToS3Key) && !empty($uploadFilesToS3Secret) &&
	...
	$res = $s3FileSys->putObject($s3Bucket, $uploadname, $localFile, '');
...
if ($result['success'] == 1) {
	...
	// Always fiel group is editor js
	$file->file_group = 'editor';
	$file->size = $f_size;
	$file->size_text = FileService::getInstance()->getReadableSize($f_size);
	$file->object_type = 'Content';
	$file->object_id = $_REQUEST['object_id'];
	$file->Save();
	if ($uploadedToS3) {
		$url = $file_url;
	} else {
