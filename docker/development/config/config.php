...
// Base URL - set via APP_BASE_URL environment variable. Dev web container is
// published on host port 5666 (see docker-compose-dev.yaml).
$baseUrl = getenv('APP_BASE_URL') ?: 'http://localhost:5666';
define('BASE_URL', $baseUrl . '/web/');
define('CLIENT_BASE_URL', $baseUrl . '/app/');

// Database configuration. Defaults match the dev mysql service in
// docker-compose-dev.yaml (user/pass dev/dev, database icehrm, host mysql).
define('APP_DB', getenv('DB_NAME') ?: 'icehrm');
define('APP_USERNAME', getenv('DB_USER') ?: 'dev');
define('APP_PASSWORD', getenv('DB_PASSWORD') ?: 'dev');
